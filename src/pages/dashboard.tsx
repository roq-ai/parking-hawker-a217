...
import { getParkingSpaces } from 'apiSdk/parking-spaces';
...
function DashboardPage() {
  ...
  const parkingSpaceStat = useSWR<PaginatedInterface<ParkingSpaceInterface>>(
    () => `/parking-spaces`,
    () => getParkingSpaces({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: parkingSpaceStat?.data?.totalCount,
      isLoading: parkingSpaceStat.isLoading,
